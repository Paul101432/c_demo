...
int main() {
    ...
    char *out = NULL;
    int nLen = strlen(str);
    printf(" test openssl begin[%s]\n", str);
    base64_encode(str, nLen, NULL, &baseLen);
    out = (char *) malloc(baseLen + 1);
    base64_encode(str, nLen, out, &baseLen);
    printf(" test openssl end [%s]\n", out);
    printf("\n");
...
ULONG AppManage(DEVHANDLE hDev, HAPPLICATION *phApp) {
    ...
    HANDLE hKey;
    unsigned char pbAuthKey[16] = {0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36,
                                   0x37, 0x38};    //初始的设备认证密钥,可以修改
    ...
    rv = SKF_CreateApplication(hDev, szAppName, "rockey", 8, "88888888", 8, SECURE_USER_ACCOUNT, phApp);
    if (rv) {
...
// 	}};
    ...
    memcpy(szContainer, "SM2_Container", 20);
...
///使用固定的SM2容器  可以枚举枚举容器，获取容器类型
    ...
    rv = SKF_ImportSessionKey(hCon, SGD_SM1_ECB, (unsigned char *) pEccCipher, sizeof(ECCCIPHERBLOB) + 16 - 1,
                              &hSessionKey);
    if (rv) {
